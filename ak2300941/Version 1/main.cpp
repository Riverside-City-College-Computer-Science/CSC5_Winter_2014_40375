...
int  find(const int [],int,int,int,int);
...
int main(int argc, char** argv) {
    ...
    //Find a value
    int val1=19,val2=17,pos=-1;
    do{
        pos=find(array,SIZE,val1,val2,pos+1);
        cout<<"Value found at "<<pos<<endl;
    }while(pos>0);
    //Declare an array and fill
    int brray[SIZE];
    find(array,SIZE,val1,val2,brray);
    //Print array
    prntAry(brray,SIZE,10);
    //Declare a vector and fill
    vector<int> posVec;
    find(array,SIZE,val1,val2,posVec);
    //Print the vector
    prntVec(posVec,10);
    //Exit Stage RIGHT!!!
    return 0;
...
void find(const int a[],int n,int val1,int val2,vector<int> &vec){
    for(int i=0;i<n;i++){
        if(a[i]==val1||a[i]==val2)vec.push_back(i);
    }
}

int  find(const int a[],int n,int val1,int val2,int pStrt){
    for(int i=pStrt;i<n;i++){
        if(a[i]==val1||a[i]==val2)return i;
    }
    return -1;
}
